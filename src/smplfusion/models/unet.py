...
class TimestepEmbedSequential(nn.Sequential, TimestepBlock):
    ...
    def forward(self, x, emb, context=None, in_mask=None, out_mask=None):
        for layer in self:
            if isinstance(layer, TimestepBlock):
                x = layer(x, emb)
            elif isinstance(layer, SpatialTransformer):
                x = layer(x, context, in_mask=in_mask, out_mask=out_mask)
            else:
                x = layer(x)
        return x
...
class ResBlock(TimestepBlock):
    ...
    def forward(self, x, emb, in_mask=None, out_mask=None):
        """
...
class UNetModel(nn.Module):
    ...
    def forward(self, x, timesteps=None, context=None, y=None, in_mask=None, out_mask=None, **kwargs):
        """
        ...
        for i, module in enumerate(self.input_blocks):
            h = module(h, emb, context, in_mask=in_mask, out_mask=out_mask)
            hs.append(h)

        h = self.middle_block(h, emb, context, in_mask=in_mask, out_mask=out_mask)

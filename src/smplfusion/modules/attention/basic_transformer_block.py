...
class PatchedBasicTransformerBlock(nn.Module):
    def __init__(
        self,dim,n_heads,d_head,dropout=0.0,context_dim=None,
        gated_ff=True,checkpoint=True,disable_self_attn=False, attn=False
    ):
        super().__init__()
        self.disable_self_attn = disable_self_attn
        # is a self-attention if not self.disable_self_attn
        ...
        self.ff = FeedForward(dim, dropout=dropout, glu=gated_ff)
        # is self-attn if context is none
        self.attn2 = CrossAttention(query_dim=dim,context_dim=context_dim,heads=n_heads,dim_head=d_head,dropout=dropout, attn=attn)
        self.norm1 = nn.LayerNorm(dim)
        self.norm2 = nn.LayerNorm(dim)
        self.norm3 = nn.LayerNorm(dim)
        self.checkpoint = checkpoint
    ...
    def forward(self, x, context=None, in_mask=None, out_mask=None):
        return checkpoint(self._forward, (x, context, in_mask, out_mask), self.parameters(), self.checkpoint)

    def _forward(self, x, context=None, in_mask=None, out_mask=None):
        return router.basic_transformer_forward(self, x, context, in_mask=in_mask, out_mask=out_mask)

...
class SpatialTransformer(nn.Module):
    ...
    def __init__(self, in_channels, n_heads, d_head,
                 ...
                 use_checkpoint=True, attn=False):
        super().__init__()
        ...
        self.transformer_blocks = nn.ModuleList(
            [BasicTransformerBlock(inner_dim, n_heads, d_head, dropout=dropout, context_dim=context_dim[d],
                                   disable_self_attn=disable_self_attn, checkpoint=use_checkpoint, attn=attn)
    ...
    def forward(self, x, context=None, in_mask=None, out_mask=None):
        ...
        for i, block in enumerate(self.transformer_blocks):
            x = block(x, context=context[i], in_mask=in_mask, out_mask=out_mask)
        if self.use_linear:
            x = self.proj_out(x)
